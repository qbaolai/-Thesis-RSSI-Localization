...
%==========================================================================
% Node Params
numOfNodes = 4; % 3 for testing trialteration
nodeRes = 8;
...
errPer = 0.25;  % odometry error percentage
% End Generate Robot
%--------------------------------------------------------------------------
...
for ii = 1:length(robotPath)
    ...
    u = [disp, (robotPose(3) - oldRobotPose(3)) + normrnd(0,0.01)];
...
% dead reckoning path
plot(dr_pose(:,1),dr_pose(:,2),'green');
for jj = 1:numOfNodes
    scatter(node(jj).pos(1),node(jj).pos(2),'blue','o');
